    ...
    .then(async () => {
        ...
        for (const model of models) {
            ...
            for (const doc of docs) {
                ...
                if (center) {
                    mappings.forEach(m => {
                        if (center.includes(m.old)) {
                            center = center.replace(m.old, m.new);
                            updated = true;
                        }
                    });
                    if (updated) doc.set('center', center);
                }

                let centerTag = doc.get('centerTag');
                if (centerTag) {
                    mappings.forEach(m => {
                        if (centerTag.includes(m.old)) {
                            centerTag = centerTag.replace(m.old, m.new);
                            updated = true;
                        }
                    });
                    if (updated) doc.set('centerTag', centerTag);
                }
